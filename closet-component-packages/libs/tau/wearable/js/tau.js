	...
	"use strict";
			...
			ns.util.easing = {
				...
				*/
				...
				/**
				* Performs in exponential calcuclations based on time
				* @method easeInExpo
				* @member ns.util.easing
				* @param {number} currentTime
				* @param {number} startValue
				...
				* @static
				*/
				easeInExpo: function (currentTime, startValue, changeInValue, duration) {
					return (currentTime === 0) ? startValue : changeInValue * (-pow(2, -10 * currentTime / duration) + 1) + startValue;
				},
				/**
				...
				easeOutExpo: function (currentTime, startValue, changeInValue, duration) {
					return (currentTime === duration) ?
							startValue + changeInValue :
								changeInValue * (-pow(2, -10 * currentTime / duration) + 1) +
								startValue;
				},
				/**
				* Performs elastic easing calculations based on time
				...
				* @static
				*/
				easeOutElastic: function (currentTime, startValue, changeInValue, duration) {
					var s = 1.70158,
						p = 0,
						a = changeInValue;
					if (currentTime === 0) {
					...
					if (a < abs(changeInValue)) {
						a = changeInValue;
						s = p / 4;
					}	else {
						s = p / (2 * PI) * asin(changeInValue / a);
					}

					return a * pow(2,-10 * currentTime) * sin((currentTime * duration - s) * (2 * PI) / p) + changeInValue + startValue;
				},
				/**
				* Performs out sine calcuclations based on time
				* @method easeOutSine
				* @member ns.util.easing
				* @param {number} currentTime
				* @param {number} startValue
				...
				*/
				easeOutSine: function (currentTime, startValue, changeInValue, duration) {
					return changeInValue * sin(currentTime/duration * (PI/2)) + startValue;
				},
				/**
...
/*global requestAnimationFrame, performance, ns */
/*
 * Copyright (c) 2015 Samsung Electronics Co., Ltd
 *
 * Licensed under the Flora License, Version 1.1 (the "License");
...
/**
 * #Effect Jelly
 * Jelly background effect for graphical eye-candy
 *
 * @author Krzysztof Antoszek <k.antoszek@samsung.com>
 * @class ns.effects
 */
(function (window, document, ns) {
	"use strict";
				// for performance reasons most of the variables are defined in module/closure scope
			// not in function scope
			var utilObject = ns.util.object,
				utilEvent = ns.event,
				utilEasing = ns.util.easing,
				easeOutElastic = utilEasing.easeOutElastic,
				easeOutSine = utilEasing.easeOutSine,
				easeInExpo = utilEasing.easeInExpo,
				interaction = false,
				elements = [],
				elementConfigs = [],
				defaultConfig = {
				...
				currentJelly = null,
				currentParent = null,
				cachedElementPosition = "",
				cachedParentPosition = "",
				currentJellyStyle = null,
				currentJellyRect = new Float32Array(4),
				mousePosition = new Float32Array(2),
				canvas = document.createElement("canvas"),
				canvasRect = new Float32Array(4),
				canvasStyle = canvas.style,
				canvasOffset = new Float32Array(2),
				context = canvas.getContext("2d"),
				horizontalPoints = 32, // horizontal resolution of effect, could be parametrized
				verticalPoints = 24, // vertical resolution of effect, could be parametrized
				markForInitialization = false,
				rendering = false,
				canvasSizeGrow = 50,
				cacheBackground = "",
				i = 0,
				l = 0,
				offsets = new Float32Array(8), // offsets of rect lines in data array
				min = Math.min,
				max = Math.max,
				sqrt = Math.sqrt,
				points = new Float32Array(),
				pointsCopy = new Float32Array(),
				forceRect = new Float32Array(4),
				forcePosition = new Float32Array(2),
				fillStyle = "",
				hasBorder = false,
				strokeStyle = "",
				currentG = defaultConfig.force,
				pointMass = 12.5, // mass for newtonian gravitation, could be parametrized
				forceMass = 15, // mass for newtonian gravitaition, could be parametrized
				renderTTL = 500, // timeout for rendering, after this the rAF siezes to run
				renderLast = 0,
				reboundTTL = 450, // elastic rebound animation time after mouseup/touchend
				reboundStart = 0,
				reboundAnimate = false,
				initialized = false;

			/**
			 * Fills suppled 2d vector (2-element) array with mouse/touch position from
			 * supplied event
			 * @method extractPosition
			 * @param {Float32Array} position
			 * @param {Event} sourceEvent
			 * @member ns.effects.jellyBackground
			 * @private
			 * @static
			 */
			function extractPosition(position, sourceEvent) {
				if (sourceEvent.type.indexOf("touch") > -1 && sourceEvent.touches.length > 0) {
					position[0] = sourceEvent.touches[0].clientX;
					position[1] = sourceEvent.touches[0].clientY;
				} else {
					position[0] = sourceEvent.clientX;
					position[1] = sourceEvent.clientY;
				}

				position[0] += currentParent.scrollLeft;
				position[1] += currentParent.scrollTop;
			}

			/**
			 * Clears the effect, moves canvas element and shows current element background
			...
			function showBackround() {
				var parent = null;
				if (currentJellyStyle) {
					currentJellyStyle.background = cacheBackground;
					currentJellyStyle.position = cachedElementPosition;
					currentJellyStyle = null;
				}

				if (currentJelly) {
					parent = currentJelly.parentNode;
					currentJelly = null;
					if (parent) {
						parent.style.position = cachedParentPosition;
					}
				}

				if (canvas.parentNode !== document.body && initialized) {
					addCanvas();
				}

				canvas.classList.remove("effect-jelly-background-active");
			}

			/**
			 * Fills destination 2d vector with position got form source 2d vector and
			 * modified by current force amount, damping and animation state.
			 * @method calculateForceVectorForPosition
			 * @param {Float32Array} src Source data flat array
			 * @param {Float32Array} dst Destination data flat array
			 * @param {Number} offset Offset for vector data
			 * @param {Number} timestamp
			 * @member ns.effects.jellyBackground
			 * @private
			 * @static
			 */
			function calculateForceVectorForPosition(src, dst, offset, timestamp) {
				var dx = 0,
					dy = 0,
					r = 0,
					f = 0,
					...
					d = 0,
					e = 0,
					G = currentConfig.force;

				if (reboundAnimate) {// swithc to gravity animation when effect ends
					reboundTime = timestamp - reboundStart;
					if (reboundTime <= reboundTTL) {
						renderLast = timestamp;
						currentG = easeOutElastic(reboundTime, G, -G, reboundTTL);// calculate elastic rebound
					} else {
						showBackround();
					}
				} else {
				...
				dx = src[offset] - forcePosition[0];
				dy = src[offset + 1] - forcePosition[1];
				r = dx * dx + dy * dy; // distance vector length between source vector and forcePosition
				f = currentG * (forceMass * pointMass / r); // newtonian gravity calculation
				d = sqrt(r); // squared distance
				e = easeInExpo(d, 0, 1, 1000); // 1000 is a magic number, need to parametrize it
				if (e === e) { // faster isNaN
					e = e * (1 - currentConfig.damp); // damping
					f *= e;
				}

				dst[offset] = src[offset] + (dx * f);
				dst[offset + 1] = src[offset + 1] + (dy * f);
			}

			/**
			 * Splits supplied distance in even parts in the requested amount
			 ...
			 * @static
			 */
			function splitDistance(distance, chunkNum) {
				var res = new Array(chunkNum),
					part = distance / (chunkNum - 1);
				for (i = 0, l = res.length - 1; i < l; ++i) {
					res[i] = part * i;
				}
				res[res.length - 1] = distance;
				return res;
			}

			/**
			 * Creates context2d color values from supplied rgb params
			 * @method rgb
			 * @param {Number} r Red
			...
			/**
			 * Translates force position vector to mouse posistion vector confinding it in
			 * force square rect
			 * @method moveForceToMouse
			 * @member ns.effects.jellyBackground
			 * @private
			 * @static
			 */
			function moveForceToMouse() {
				// translate to proper canvas size and position offset
				forcePosition[0] = mousePosition[0] - canvasRect[0] + canvasOffset[0];
				forcePosition[1] = mousePosition[1] - canvasRect[1] + canvasOffset[1];
				// trap in canvas size
				forcePosition[0] = max(0, min(forcePosition[0], canvasRect[2]));
				forcePosition[1] = max(0, min(forcePosition[1], canvasRect[3]));
				// trap in force rect
				forcePosition[0] = max(forceRect[0] + canvasOffset[0], min(forcePosition[0], forceRect[2] + canvasOffset[0]));
				forcePosition[1] = max(forceRect[1] + canvasOffset[1], min(forcePosition[1], forceRect[3] + canvasOffset[1]));
			}

			/**
			 * Fills an array of element dimensions based on offsets since getClientBoundingRect
			 * returns inconsistent results
			 * @method getElementRect
			 * @param {Element} element
			 * @param {Flaot32Array} result
			 * @member ns.effects.jellyBackground
			 * @private
			 * @static
			 */
			function getElementRect(element, result) {
				result[0] = result[1] = 0;
				result[2] = element.offsetWidth;
				result[3] = element.offsetHeight;
			...
			/**
			 * Performs initial effect initialization for current element. Sets sizes, force
			 * position, rects, etc.
			 * @method organize
			 * @member ns.effects.jellyBackground
			 * @private
			 ...
			 */
			function organize() {
				var width = currentJellyRect[2] - currentJellyRect[0],
					height = currentJellyRect[3] - currentJellyRect[1],
					destWidth = width + canvasSizeGrow,
					destHeight = height + canvasSizeGrow,
					widthParts = splitDistance(width, horizontalPoints),
					heightParts = splitDistance(height, verticalPoints),
					parent = currentJelly.parentNode,
					computed = window.getComputedStyle(currentJelly, null),
					computedParent = window.getComputedStyle(parent, null),
					k = 0,
					tmpData = [],
					damp = currentConfig.damp,
					widthFactor = width > height ? 4 : 2,
					heightFactor = width > height ? 2 : 4,
					spacing = canvasSizeGrow / 2,
					parentWidth = parent.clientWidth,
					parentHeight = parent.clientHeight;

				fillStyle = computed.getPropertyValue("background-color");
				hasBorder = !!parseInt(computed.getPropertyValue("border-width"));
				strokeStyle = computed.getPropertyValue("border");
				cacheBackground = currentJelly.style.background;
				currentJellyStyle.background = "transparent none";
				cachedElementPosition = computed.getPropertyValue("position");
				cachedParentPosition = computedParent.getPropertyValue("position");
				// current element ha to have a position other than static for proper display
				// stacking
				if (cachedElementPosition === "static" || cachedElementPosition.length === 0) {
					currentJellyStyle.position = "relative";
				}
				// parent element has to have a position other than static for proper display
				// stacking
				if (cachedParentPosition === "static" || cachedParentPosition.length === 0) {
					parent.style.position = "relative";
				}

				canvasStyle.width = (canvas.width = destWidth) + "px";
				canvasStyle.height = (canvas.height = destHeight) + "px";

				// set canvas rect based on fetched sizes and expand it according to grow factor
				canvasRect[0] = currentJellyRect[0] - canvasSizeGrow / 2;
				canvasRect[1] = currentJellyRect[1] - canvasSizeGrow / 2;
				canvasRect[2] = canvasRect[0] + destWidth;
				canvasRect[3] = canvasRect[1] + destHeight;

				// clear canvas translation offsets
				canvasOffset[0] = canvasOffset[1] = 0;

				if (parentWidth < destWidth) { // if element is first in contianer
					canvasOffset[0] = (parentWidth - destWidth) / 2;
					canvasStyle.width = (canvas.width = parentWidth) + "px";
					canvasStyle.left = "0";
				} else if (currentJellyRect[0] < canvasSizeGrow / 2) { // if element is last in container
					canvasOffset[0] = currentJellyRect[0] - canvasSizeGrow / 2;
					canvasStyle.width = (canvas.width = destWidth - canvasSizeGrow / 2) + "px";
					canvasStyle.left = "0";
				} else {
					canvasStyle.left = (currentJellyRect[0] - canvasSizeGrow / 2)+ "px";
					canvasStyle.width = (canvas.width = destWidth) + "px";
				}

				if (parentHeight < destHeight) { // if element is first in container
					canvasOffset[1] = (parentHeight - destHeight);
					canvasStyle.height = (canvas.height = parentHeight) + "px";
					canvasStyle.top = "0";
				} else if (currentJellyRect[1] < canvasSizeGrow) { // if element is last in container
					canvasOffset[1] = (currentJellyRect[1] - canvasSizeGrow / 2);
					canvasStyle.height = (canvas.height = destHeight - canvasSizeGrow / 2) + "px";
					canvasStyle.top = "0";
				} else {
					canvasStyle.top = (currentJellyRect[1] - canvasSizeGrow / 2) + "px";
					canvasStyle.height = (canvas.height = destHeight) + "px";
				}

				canvas.classList.add("effect-jelly-background-active");

				// offset start for top line vector data is always 0
				// fill top line vectors
				for (i = 0, l = widthParts.length; i < l; ++i) {
					tmpData.push(widthParts[i] + spacing);
					tmpData.push(heightParts[0] + spacing);
				}
				// get end offset for top line vector data
				offsets[1] = tmpData.length - 1;
				// get start offset for right line vector data
				offsets[2] = offsets[1] + 1;
				// fill right line vectors
				for (i = 0, l = heightParts.length, k = widthParts.length - 1; i < l; ++i) {
					tmpData.push(widthParts[k] + spacing);
					tmpData.push(heightParts[i] + spacing);
				}
				// get end offset for right line vector data
				offsets[3] = tmpData.length - 1;
				// get start offset for bottom line vector data
				offsets[4] = offsets[3] + 1;
				// fill bottom line vectors
				for (i = widthParts.length - 1, k = heightParts.length - 1; i >= 0; --i) {
					tmpData.push(widthParts[i] + spacing);
					tmpData.push(heightParts[k] + spacing);
				}
				// get end offset for bottom line vector data
				offsets[5] = tmpData.length - 1;
				// get start offset for left line vector data
				offsets[6] = offsets[5] + 1;
				// fill left line vectors
				for (i = heightParts.length - 1, k = widthParts.length - 1; i >= 0; --i) {
					tmpData.push(widthParts[0] + spacing);
					tmpData.push(heightParts[i] + spacing);
				}
				// get end offset for left line vector data
				offsets[7] = tmpData.length - 1;

				// copy vectors to more efficiend datatype
				points = new Float32Array(tmpData);
				pointsCopy = new Float32Array(tmpData);

				// force rect along with distance easing (in the calculate force function)
				// are responsible for damping, this will be parametrized
				forceRect[0] = widthParts[0] + (width / widthFactor * damp);
				forceRect[1] = heightParts[0] + (height / heightFactor * damp);
				forceRect[2] = widthParts[widthParts.length - 1] - (width / widthFactor * damp);
				forceRect[3] = heightParts[heightParts.length - 1] - (height / heightFactor * damp);
				for (i = 0; i < 4; ++i) {
					forceRect[i] += spacing;
				}

				// place force at center
				forcePosition[0] = width / 2 + spacing;
				forcePosition[1] = height / 2 + spacing;

				// try to move it also according to mouse
				moveForceToMouse();
			...
			/**
			 * Smooths out irregularities on supplied line 2d vector data
			 * @method smoothLine
			 * @param {Float32Array} pointsArr
			 * @param {Number} start Line starting offset
			 * @param {Number} end Line ending offset
			 * @param {Number} side Supplied side (0: top, 1: right, 2: botto, 3: left)
			 * @member ns.effects.jellyBackground
			 * @private
			 * @static
			 */
			function smoothLine(pointsArr, start, end, side) {
				var neg = [0, 3].indexOf(side) > -1, // less is more
					max = neg ? 1000 : -1,
					j = 0,
					idx = -1,
					dir = side % 2, // get direction from side, 0: horizontal, 1: vertical
					v = 0,
					diff = 0;
				...
				for (i = start; i < end; i += 2) {
					j = dir === 0 ? i + 1 : i;
					if ((neg && max > pointsArr[j]) || (!neg && max < pointsArr[j])) {
						max = pointsArr[j];
						idx = i;
					}
				}
				...
				// modify line
				for (i = start, l = dir === 0 ? start + 1 : start; i < end; i += 2) {
					j = dir === 0 ? i + 1 : i;
					if (idx > i) { // low->high pass before highest point
						v = easeOutSine(i - start, points[l], max - points[l], idx - start);
					} else if (idx < i) { // high->low after highest point
						v = easeOutSine(end - i, points[l], max - points[l], end - idx);
					} else {
						v = max;
					}

					// for some points values can get lower/higher than originating values
					// we need to fix this here by using old value
					if ((neg && v > pointsArr[j]) || (!neg && v < pointsArr[j])) {
						v = pointsArr[j];
					}

					diff = v - pointsArr[j];

					pointsArr[j] += diff * currentConfig.smoothing;
				}
			}

			/**
			 * Applies line smooting for all sides
			 * @method applyLineSmooting
			 * @member ns.effects.jellyBackground
			 * @private
			 * @static
			 */
			function applyLineSmoothing() {
				smoothLine(pointsCopy, offsets[0], offsets[1], 0);
				smoothLine(pointsCopy, offsets[2], offsets[3], 1);
				smoothLine(pointsCopy, offsets[4], offsets[5], 2);
				smoothLine(pointsCopy, offsets[6], offsets[7], 3);
			}

			/**
			 * Applies newtonian gravitational force for every 2d vector
			 * @method applyForceToPoints
			 * @param {Number} timestamp
			 * @member ns.effects.jellyBackground
			 * @private
			 * @static
			 */
			function applyForceToPoints(timestamp) {
				for (i = 0, l = points.length; i < l; i += 2) {
					calculateForceVectorForPosition(points, pointsCopy, i, timestamp);
				}
			}
			...
			function render(timestamp) {
				if (markForInitialization) {
					organize();
				}
				applyForceToPoints(timestamp);
				applyLineSmoothing(timestamp);

				canvas.width = canvas.width; // fast clear of canvas
				context.save();
				context.translate(canvasOffset[0], canvasOffset[1]);

				context.save();
				context.beginPath();
				context.moveTo(points[0], points[1]);
				for (i = 0, l = points.length; i < l; i += 2) {
					context.lineTo(pointsCopy[i], pointsCopy[i + 1]);
				}
				context.closePath();
				context.fillStyle = fillStyle;
				context.fill();
				if (hasBorder) { // support for border colors
				...
				if (currentConfig.debug) {
					...
					for (i = 0, l = points.length; i < l; i += 2) {
						context.beginPath();
						context.rect(pointsCopy[i] - 1, pointsCopy[i + 1] - 1, 3, 3);
						context.closePath();
						context.fill();
						context.stroke();
					}
					...
					context.beginPath();
					context.rect(forceRect[0] - 1, forceRect[1] - 1, forceRect[2] - forceRect[0] + 2, forceRect[3] - forceRect[1] + 2);
					context.closePath();
					context.stroke();
					context.restore();
					...
					context.strokeStyle = rgb(255, 255, 0);
					context.beginPath();
					context.rect(forcePosition[0] - 2, forcePosition[1] - 2, 5, 5);
					context.closePath();
					context.fill();
					context.restore();
					...
					context.beginPath();
					context.rect(1, 1, canvasRect[2] - canvasRect[0] - 1, canvasRect[3] - canvasRect[1] - 1);
					context.closePath();
					context.stroke();
					context.restore();
				}
				...
				if (timestamp - renderLast < renderTTL) {
			...
			/**
			 * Traverses the element parent tree to find if any element has the effect
			 * enabled
			 * @method findWatchedElement
			 * @param {Element} element
			 * @member ns.effects.jellyBackground
			 * @private
			 * @static
			 */
			function findWatchedElement(element) {
				var index = -1;
				while (element && element !== document) {
			...
			/**
			 * Gets configuration for supplied element
			 * @method getConfigForElement
			 * @param {Element} element
			 * @return {Object}
			 * @member ns.effects.jellyBackground
			 * @private
			...
			/**
			 * Handles mousedown/touch start, effect initialization and render loop starting
			...
			function handleMouseDown(event) {
				var target = findWatchedElement(event.target),
					rect = null;
				...
				if (target) {
					...
					currentJellyStyle = target.style;
					utilObject.fastMerge(currentConfig, getConfigForElement(target)); // reuseable objects
					rect = currentJelly.getBoundingClientRect();
					getElementRect(currentJelly, currentJellyRect);
					extractPosition(mousePosition, event);
					currentJelly.addEventListener("dragend", handleMouseUp, false);
					markForInitialization = true;
					reboundAnimate = false;
					if (initialized) {
						currentParent.insertBefore(canvas, currentJelly);
					}
					if (!rendering) {
						renderLast = performance.now(); // get high res timestamp
						requestAnimationFrame(render);
					}
				}
			}
			...
			/**
			 * Handles mouse position tracking if any effect is running
			...
			function handleMouseMove(event) {
				if (interaction) {
					...
					moveForceToMouse();
					if (!rendering) {
						renderLast = reboundStart = performance.now(); // get high res timestamp
						requestAnimationFrame(render);
					}
				}
			}
			...
			/**
			 * Handles effect clearing when mouse/touch is released
			...
			function handleMouseUp(event) {
				if (interaction) {
					...
					if (event.type === "dragend") {
						event.target.removeEventListener("dragend", handleMouseUp, false);
					}
				}
				interaction = false;
			}
			 ...
			 */
			function addCanvas() {
				canvas.className = "effect-jelly-background";
				document.body.appendChild(canvas);
			}

			/**
			 * Removes canvas from body
			 * @method removeCanvas
			 * @member ns.effects.jellyBackground
			 * @private
			 * @static
			 ...
			 */
			function bindEvents() {
				utilEvent.on(document, "mousedown touchstart", handleMouseDown);
				utilEvent.on(document, "mousemove touchmove", handleMouseMove);
				utilEvent.on(document, "mouseup touchend", handleMouseUp);
			}
			...
			function releaseEvents() {
				utilEvent.off(document, "mousedown touchstart", handleMouseDown);
				utilEvent.off(document, "mousemove touchmove", handleMouseMove);
				utilEvent.off(document, "mouseup touchend", handleMouseUp);
			}


			/**
			 * Sets configuration on for supplied element
			 * @method setOptions
			 * @param {Element} element
			 * @param {Object} config Configuration
			 * @param {Number} [config.damp=0.4] Effect damping
			 * @param {Number} [config.smoothing=0.5] Effect line smoothing
			 * @param {Number} [config.force=8] The amount of force
			 * @param {Boolean} [config.debug=false] If set to true, debug lines will be drawed
			 * @member ns.effects.jellyBackground
			 * @static
			 */
			function setOptions(element, options) {
				var index = -1;
				if ((index = elements.indexOf(element)) > -1 && options) {
					utilObject.fastMerge(elementConfigs[index], options);
				}
			}

			/**
			 * Turns effect on supplied element
			 * @method on
			 * @param {Element} element
			 * @param {Object} config Configuration
			 * @param {Number} [config.damp=0.4] Effect damping
			 ...
			 * @static
			 */
			function on(element, config) {
				if (elements.indexOf(element) === -1) {
					elements.push(element);
					elementConfigs.push(utilObject.fastMerge({}, config || defaultConfig));
				}
			...
			/**
			 * Turns effect off for supplied element
			 * @method off
			 * @param {Element} element
			 * @member ns.effects.jellyBackground
			 * @static
...
(function (window, document, ns) {
	"use strict";
			...
			function addPath(svg, options) {
				...
				path.setAttribute("d", describeArc(options.x, options.y, options.r, options.arcStart, options.arcEnd));
...
/*global cancelAnimationFrame, requestAnimationFrame, performance, ns, createjs*/
/**
 * #Merging bubbles effect
 *
 * @author Krzysztof Antoszek <k.antoszek@samsung.com>
 * @author Piotr Karny <p.karny@samsung.com>
 * @uses TweenJS library
 *
 * @class ns.effects.mergingBubbles
 */
(function (window, document, ns) {
	"use strict";
			...
			var sqrt = Math.sqrt,
				...
				pow = Math.pow,
				asin = Math.asin,
				polarToCartesian = ns.util.polar.polarToCartesian,
		...
		float menuDiameter = #menuDiameter#.;\n\
		float sum = 0.;\n\
		float pixelPower = 0.;\n\
		float maxPower = 0.;\n\
		float shrinkThreshold = .4;\n\
		float calibration = 0.;\n\
		\n\
		vec3 point;\n\
		...
		\n\
		uniform vec3 uCircles[#circlesNum#];\n\
		uniform vec3 uColors[#circlesNum#];\n\
		uniform vec3 uCenterColor;\n\
		uniform float uCenterCircleRadius;\n\
		uniform float uAnimTime;\n\
		\n\
		float metaball(vec2 point, vec3 circle) {\n\
			return (circle.z) / min(distance(point.xy, circle.xy),.5);\n\
		}\n\
		float easeInCubic(float change, float cval, float cmax, float start) {\n\
			cval = cval / cmax;\n\
			return change * cval * cval * cval + start;\n\
		}\n\
		float easeInExpo(float change, float cval, float cmax, float start) {\n\
			if (cval == 0.0) {\n\
				return start;\n\
			}\n\
			return change * pow(2., 10. * (cval/cmax - 1.)) + start;\n\
		}\n\
		\n\
		void main(void) {\n\
			point = vec3(gl_FragCoord.xy / menuDiameter, 0.);\n\
			point.y = 1. -point.y;\n" +
"			for (int i = 0; i < #circlesNum#; ++i) {\n\
				pixelPower = metaball(point.xy, uCircles[i] / menuDiameter);\n\
				if (maxPower < pixelPower) {\n\
					maxPower = pixelPower;\n\
					vColor = uColors[i];\n\
				}\n\
				sum += pixelPower;\n\
			...
			for (int i = 0; i < #circlesNum#; ++i) {\n\
				pixelPower = metaball(point.xy, uCircles[i]/ menuDiameter);\n\
				vColor = mix(vColor, uColors[i].rgb, easeInCubic(1.0, pixelPower / maxPower, 1., 0.));\n\
				//vColor = mix(vColor, uColors[i].rgb, uAnimTime);\n\
			}\n\
			if (uAnimTime > shrinkThreshold) {\n\
                calibration = easeInExpo(1., uAnimTime - shrinkThreshold, 1. - shrinkThreshold, 0.) * #calibrationConst#;\n\
			}\n\
			if (sum > #sumConst# + calibration) {\n\
				if (distance(point.xy, center.xy) <= uCenterCircleRadius / menuDiameter) {\n\
					vColor = uCenterColor;\n\
				} else {\n\
					vColor = mix(vColor, uCenterColor, easeInExpo(1., (uCenterCircleRadius / menuDiameter) / distance(point.xy, center.xy), 1., 0.));\n\
				}\n\
		...
		",
			/*jshint +W043 */
				...
				MergingBubbles = function (container, options) {
					var bubbles = [].slice.call(container.children),
						bubblesLength = bubbles.length;

					if (bubblesLength === 0) {
						ns.error("Menu does not have any items");
						return;
					}
					...
					// Lazy way to define object properties
					ns.util.object.merge(this, {
						container: container,
						bubbles: bubbles,
						options: {
							autoPosition: (options.autoPosition !== undefined) ? options.autoPosition : true,
							/**
							 * auto - positions bubbles on circle around middle of the container
							 * arc - positions bubbles on arc shape with center in the middle of the container (auto distance between bubbles)
							 * custom - turns off auto position, user defines own positions
							 * [number] - allows to define number of places to fit bubbles in (angleBetweenBubbles = 360 / number)
							 */
							distribution: options.distribution || "auto", // auto, arc, custom, [number]
							elasticity: options.elasticity || 50,
							viscosity: options.viscosity || 0.5,
							duration: (typeof options.duration === "number") ? max(options.duration, 0) : 1000,
							startAngle: options.startAngle || 0,
							animationDelay: options.animationDelay || 0,
							direction: options.direction === -1 ? -1 : 1,
							/**
							 * {Array} [centerColor=[90,90,90]] Color of center defined in RGB
							 */
							centerColor: options.centerColor || [90, 90, 90]
						},
						canvas: document.createElement("canvas"),
						currentBubble: null,
						gl: null,
						...
						animationFrame: null,
						/**
						 * Current position of bubbles
						 */
						uCircles: new Float32Array(bubblesLength * 3),
						uColors: new Float32Array(bubblesLength * 3),
						/**
						 * Color of center bubble
						 */
						uCenterColor: new Float32Array(3),

						uColorsAddr: null,
						uCirclesAddr: null,
						uCenterColorAddr: null,
						uCenterCircleRadiusAddr: null,
						uAnimTimeAddr: null,
						...
						direction: 0,
						/**
						 * Start position of all bubbles
						 */
						positions: new Float32Array(bubblesLength * 5),
						ct: 0,
						_renderLoop: null,
						_clickHandler: null
					});
			...
			function easeOut(change, cval, cmax, start) {
				return -change * (cval /= cmax) * (cmax - 2) + start;
			}

			function easeOutCustom(t, b, c, d) {
				var ts=(t/=d)*t;
				var tc=ts*t;
				return b+c*(-0.2975*tc*ts + 0.4975*ts*ts + -0.1*tc + -0.5*ts + 1.4*t);
			}

			function easeInCustom(t, b, c, d) {
				var ts=(t/=d)*t;
				var tc=ts*t;
				return b+c*(0.1*tc*ts + -0.15*ts*ts + 0.1*tc + 0.1*ts + 0.85*t);
			}

			function fetchContext(canvas) {
				try {
					return canvas.getContext("webgl", WEBGL_CONF) ||
						canvas.getContext("experimental-webgl", WEBGL_CONF);
				} catch (e) {
					ns.error("WebGL not supported");
				}

				return false;
			}

			function fillColor(element, data, dataOffset) {
				var style = window.getComputedStyle(element, null),
					c = style.getPropertyValue("background-color").match(/\s*(\d+),\s*(\d+)\s*,\s*(\d+)\s*(,\s*([0-9\.]+)?\s*)?/i);

				if (c) {
					data[dataOffset] = parseFloat(c[1]) / 255 || 0.0;
					data[dataOffset + 1] = parseFloat(c[2]) / 255 || 0.0;
					data[dataOffset + 2] = parseFloat(c[3]) / 255 || 0.0;
				}
			}

			function fillManualPositionAndSize(element, data, dataOffset) {
				var rect = element.getBoundingClientRect(),
					width = rect.width,
					height = rect.height,
					// Radius will be the a circle inside of the element
					r = min(width, height) / 2;

				data[dataOffset] = rect.left + r;
				data[dataOffset + 1] = rect.top + r;
				data[dataOffset + 2] = r;
			}

			function fillAutoPositionAndSize(element, data, dataOffset, index, bubbleCount, canvasWidth, canvasHeight, options) {
				var rect = element.getBoundingClientRect(),
					width = rect.width,
					height = rect.height,
					centerX = canvasWidth / 2,
					centerY = canvasHeight / 2,
					angleDirection = options.direction,
					// Bubbles will be placed on a circle path with the radius
					// of 35% of smalled dimension of the canvas
					bubblesPositionRadius = (min(canvasWidth, canvasHeight) * 0.35),
					// Radius will be the a circle inside of the element
					r = min(width, height) / 2,
					...
					prevElementRadius = data[dataOffset - 1] !== undefined ? data[dataOffset - 1] : r; // dataOffset - 3 + 2

				// Save the radius
				data[dataOffset + 2] = r;

				// Set position based on current index and desired design
				if (distribution === "arc") {
					// Every bubble should be placed in degrees * index from start position
					// Bubbles may have different size but we want to have some distance between them
					// to ensure this we assume that distance from bubbles must be at least
					// (radiusOfPreviousElement + radiusOfCurrent) * 2 (this cannot be greater than radius
					// to effect center).
					// ASin will then return the radian degrees which are needed for placing the bubble
					//    O---*---O - bubbles
					//   / \  |  / \
					//  /   \ | /
					// /     \|/<- 2 * result of asin calculation == angleValue
					// 0      . - center
					// \
					angleValue = (asin(min(prevElementRadius + r,  bubblesPositionRadius) / bubblesPositionRadius) * RAD_TO_DEG) * 2;
				} else if (typeof distribution === "number") {
					angleValue = 360 / distribution;
				} else {
					// including "auto" - default option
					angleValue = 360 / bubbleCount;
				}

				angleValue = (index * angleValue * angleDirection) + options.startAngle;

				cartesianCoordinates = polarToCartesian(centerX, centerY, bubblesPositionRadius, angleValue);

				data[dataOffset] = cartesianCoordinates.x;
				data[dataOffset + 1] = cartesianCoordinates.y;

				// Also position the bubble correctly
				elementStyle.left = cartesianCoordinates.x - (width / 2) + "px";
				elementStyle.top = cartesianCoordinates.y - (height / 2) + "px";
			}

			function renderLoop(self, time) {
				var gl = self.gl;

				gl.viewport(0, 0, self.width, self.height);
				gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT);
				gl.bindBuffer(gl.ARRAY_BUFFER, self.plane);
				gl.vertexAttribPointer(self.aVertexPosA, 3, gl.FLOAT, false, 0, 0);
			...
			function handleClick(self, e) {
				var menuItem = null;
				if (!self.animating) {
					menuItem = goToMenuItem(e.target);
					self.animate(menuItem);
				}
			}

			function animationComplete(self, index) {
				// If all animation have finished
				if (index === self.bubbles.length - 1) {
					self.animating = false;
					if (self.direction === 0 && self.currentBubble) {
						self.currentBubble.classList.remove("current");
					}
				}
			}
			...
			prototype = {
				/**
				 * Writes values from current state into gl context using
				 * uniform addresses fetched earlier
				 * @protected
				 */
				_applyUniforms: function () {
					...
					gl.uniform3fv(self.uColorsAddr, self.uColors);
					gl.uniform3fv(self.uCirclesAddr, self.uCircles);
					gl.uniform3fv(self.uCenterColorAddr, self.uCenterColor);

					gl.uniform1f(self.uCenterCircleRadiusAddr, self.uCenterCircleRadius);
					gl.uniform1f(self.uAnimTimeAddr, self.normalizedAnimTime);
				},
				/**
				 * Compiles the program for GL  using two shaders.
				 * This method also retrieves the addresses of uniforms
				 * @param {WebGLShader} vertexShader
				 * @param {WebGLShader} fragmentShader
				 * @returns {WebGLProgram|null}
				 * @private
				 */
				_createShaderProgram: function (vertexShader, fragmentShader) {
					var self = this,
						program = null,
						gl = self.gl;

					if (!vertexShader) {
						ns.error("No vertex shader specified!");
						return null;
					}

					if (!fragmentShader) {
						ns.error("No fragment shader specified");
						return null;
					}
					...
					if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
						ns.error("Could not initialize shader program!");
						return null;
					}

					self.aVertexPosA = gl.getAttribLocation(program, "aVertexPosition");
					gl.enableVertexAttribArray(self.aVertexPosA);

					self.uColorsAddr = gl.getUniformLocation(program, "uColors");
					self.uCirclesAddr = gl.getUniformLocation(program, "uCircles");
					self.uCenterColorAddr = gl.getUniformLocation(program, "uCenterColor");
					self.uCenterCircleRadiusAddr = gl.getUniformLocation(program, "uCenterCircleRadius");
					self.uAnimTimeAddr = gl.getUniformLocation(program, "uAnimTime");

					return program;
				},
				/**
				 * Compiles shader code with variables set (using string
				 * replacement).
				 * @param {MergingBubbles} self
				 * @param {number} type
				 * @param {string} code
				 ...
				 * @protected
				 */
				_compileShader: function (self, type, code, variables) {
					var shader = null,
						source = code,
						prop = "",
						gl = self.gl;
					...
					if (type !== VERTEX_SHADER && type !== FRAGMENT_SHADER) {
						ns.error("Improper shader type");
						return null;
					}

					if (!source || source.length < 1) {
						ns.error("No shader code supplied");
						return null;
					}
					...
					if (variables) {
						for (prop in variables) {
							if (variables.hasOwnProperty(prop)) {
								source = source.replace(new RegExp("#" + prop + "#", "gi"), variables[prop]);
							}
						}
					}
					...
					if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
						ns.log(source);
						ns.error(gl.getShaderInfoLog(shader));
						return null;
					}
				...
				/**
				 * Initializes GL context with compiled and loaded shader program
				 * @returns {boolean}
				 * @protected
				 */
				_initializeGL: function () {
					var self = this,
						gl = null,
						bubbleLength = self.bubbles.length,
						viscosity = min(1, max(0, 1 - self.options.viscosity));

					if (!(gl = fetchContext(self.canvas))) {
						return false;
					}

					self.gl = gl;

					if (!(self.shaderProgram = self._createShaderProgram(
							self._compileShader(self, VERTEX_SHADER, VERTEX_SHADER_CODE),
							self._compileShader(self, FRAGMENT_SHADER, FRAGMENT_SHADER_CODE, {
								circlesNum: bubbleLength,
								menuDiameter: min(self.width, self.height),
								// Constants manually set to match defined size
								// any changes to screen size must later be changed here
								sumConst: 1.0 + easeInCustom(bubbleLength - 1, 0.0, 2.1 * (2 * viscosity), 8),
								calibrationConst: easeOutCustom(bubbleLength - 1, 0.0, 6.3, 9)
							})))) {
						return false;
					}
				...
				/**
				 * Retrives data about bubbles described with HTML, stores them
				 * in internal table and prefills all position tables
				 * @returns {boolean}
				 * @@protected
				 */
				_initializeBubbles: function () {
					var i = 0,
						bubbles = this.bubbles,
						l = bubbles.length,
						uColors = this.uColors,
						uCircles = this.uCircles,
						positions = this.positions,
						options = this.options,
						manualPosition = options.distribution === "custom",
						bubbleRadiusSum = 0,
						// Fetched here to save time later
						height = this.height,
						width = this.width,
						dataOffset = 0;
					...
					for (; i < l; ++i) {
						dataOffset = i * 3;
						fillColor(bubbles[i], uColors, dataOffset);

						if (manualPosition) {
							fillManualPositionAndSize(bubbles[i], uCircles, dataOffset);
						} else {
							fillAutoPositionAndSize(bubbles[i], uCircles, dataOffset, i, l, width, height, options);
						}

						// Rewrite to positions table
						positions[dataOffset] = uCircles[dataOffset];
						positions[dataOffset + 1] = uCircles[dataOffset + 1];
						positions[dataOffset + 2] = uCircles[dataOffset + 2];

						bubbleRadiusSum += uCircles[dataOffset + 2];

						bubbles[i].style.backgroundColor = "transparent";
					}

					this.uCenterCircleRadius = bubbleRadiusSum / l;

					return true;
				},
				/**
				 * Creates and binds buffer data to store vertices
				 * @protected
				 */
				_initializePlane: function () {
					var gl = this.gl;

					this.plane = gl.createBuffer();
				...
				/**
				 * Initialize effect.
				 * Sets default values for internal properties, bubbles and GL
				 * @returns {boolean}
				 */
				initialize: function () {
					var self = this,
						...
						centerX = 0,
						centerY = 0,
						uCenterColor = self.uCenterColor,
						optionsCenterColor = self.options.centerColor,
						canvas = self.canvas,
						canvasStyle = canvas.style,
						container = self.container;

					if (!createjs) {
						ns.error("Cannot initialize Merging Bubbles effect, " +
							"cannot find 'createjs' object. Effect requires " +
							"'TweenJS' library, make sure it's included");
						return false;
					}

					Tween = createjs.Tween;

					containerStyle = container.getBoundingClientRect();

					width = containerStyle.width || window.innerWidth;
					height = containerStyle.height || window.innerHeight;

					centerX = width / 2;
					centerY = height / 2;
					uCenterColor[0] = optionsCenterColor[0] / 255;
					uCenterColor[1] = optionsCenterColor[1] / 255;
					uCenterColor[2] = optionsCenterColor[2] / 255;
					...
					// Modified later based on average of radius
					self.uCenterCircleRadius = 0;
					...
					canvas.className = "ui-bubble-menu-canvas";

					container.parentElement.insertBefore(canvas, container);

					if (!self._initializeBubbles(container)) {
						ns.error("Something went wrong when initializing menu items");
						return false;
					}

					if (!self._initializeGL(width, height)) {
						ns.error("Something went wrong when initializing GL");
						return false;
					}

					self._initializePlane();

					// Prepare render function
					self._renderLoop = renderLoop.bind(null, self);

					// Save reference to handler to be able to remove it later
					self._clickHandler = handleClick.bind(null, self);

					container.addEventListener("click", self._clickHandler, false);

					return true;
				},
				/**
				 * Updates logic based on current animation time
				 */
				update: function() {
					var self = this,
						i = 0,
						...
						circleOffset = 0,
						animOffset = 0,
						uCircles = self.uCircles,
						animationStartCt = self.animationStartCt,
						uCenterCircle = self.uCenterCircle,
					...
					if(self.animating && tween) {

						ctDelta = self.ct - animationStartCt;

						if (self.direction === 1) {
							self.normalizedAnimTime = min(ctDelta / tween.duration, 1);
						} else {
							self.normalizedAnimTime = max(1 - (ctDelta / tween.duration), 0);
						}

						tween.setPosition(ctDelta);

						for (; i < l; ++i) {
							circleOffset = i * 3;
							animOffset = i * 4;
							menuElement = bubbles[i];
							style = menuElement.style;

							style.transform = "translate3d(" + (uCircles[circleOffset] - positions[circleOffset]) + "px, " + (uCircles[circleOffset + 1] - positions[circleOffset + 1]) + "px, 0px)";

							if (!menuElement.classList.contains("current")) {
								distance = d(uCircles[circleOffset], uCircles[circleOffset + 1], centerX, centerY);
								if (distance > uCircles[circleOffset + 2]) {
									style.opacity = easeOut(-1, self.normalizedAnimTime, 1.0, 1);
								} else {
									style.opacity = 0;
								}
							}
				...
				/**
				 * Sets animation parameters: target position, direction, states
				 * based on given element
				 * @method animate
				 * @param {HTMLElement} selectedBubble
				 * @public
				 */
				animate: function(selectedBubble) {
					var self = this,
						cdir = self.direction,
						uCircles = self.uCircles,
						positions = self.positions,
						...
						tweens = [],
						circleOffset = 0,
						bubblesCount = self.bubbles.length,
						i = 0,
						options = self.options,
						animationDelay = options.animationDelay,
						duration = options.duration,
						tweenTimelineSettings = {
								paused: true
							};
					...
					if (!selectedBubble) {
						...
						cdir = selectedBubble.classList.contains("current") ? 0 : 1;
					}

					if (cdir !== self.direction) {
						self.animating = true;
						self.animationStartCt = self.ct;

						for(;i < bubblesCount; i++) {
							...
							animationFinalState[circleOffset] = cdir === 0 ? positions[circleOffset] : self.centerX;
							animationFinalState[circleOffset + 1] = cdir === 0 ? positions[circleOffset + 1] : self.centerY;

							// If we have to set animationDelay we need to push every tween into queue
							// in case animationDelay is 0, we only need to push it once
							if (animationDelay > 0 || i === bubblesCount - 1) {
								tweens.push(Tween.get(uCircles, tweenTimelineSettings)
									.wait(i * animationDelay)
									.to(animationFinalState, duration, createjs.Ease.getBackOut(2.5))
									.call(animationComplete, [self, i]));
							}
						}

						if (cdir === 1) {
							self.currentBubble = selectedBubble;
							selectedBubble.classList.add("current");
						}
						self.direction = cdir;
					}

					self.tween = new createjs.Timeline(tweens, null, {
						paused: true
					});
				},
				refresh: function () {
					// @TODO reposition nodes, recompile shader in case children count or attributes change
					ns.warn(".refresh method is not implemented");
				},
				/**
				 * Renders the current state of objects onto scene
				 * @method render
				 * @public
				 */
				render: function () {
				...
				/**
				 * Removes canvas and cancels animation
				 * @method destroy
				 * @public
				 */
				destroy: function () {
					...
					self.canvas.parentElement.removeChild(self.canvas);

					self.container.addEventListener("click", self._clickHandler, false);
					self._clickHandler = null;
				}
			};

			MergingBubbles.prototype = prototype;

			/**
			 * Creates and initializes instance of Merging Bubbles effect
			 * @param {HTMLElement} container
			 * @param {object} options
			 * @returns {MergingBubbles}
			 */
			function create(container, options) {
				var menu = new MergingBubbles(container, options);
				menu.initialize();

				return menu;
			}

			ns.effects.mergingBubbles = {
				create: create
			};
...
(function (document, ns) {
	"use strict";
			...
			prototype._configure = function () {
				var self = this;
				self.options = self.options || {};
				objectUtil.fastMerge(self.options, {
					// common options
					inline: false,//url
					icon: null,
					disabled: false,
...
(function (document, ns) {
	"use strict";
			 ...
			 * @static
			 */
			Button.events = CoreButton.events || {};
 ...
 * The button widget shows a control on the screen that you can use to generate an action event when it is pressed and released. This widget is coded with standard HTML anchor and input elements.
 *
 * Default selector for buttons is class *ui-btn-jelly*
 *
 * ### HTML Examples
 *
 * #### Standard button
 ...
 *
 *      @example
 *      <button type="button" class="ui-btn-jelly">Button</button>
 *      <a href="#" class="ui-btn-jelly">Button</a>
 *      <input type="button" class="ui-btn-jelly" value="Button" />
 *
 * #### Inline button
 *
 *      @example
 *      <input type="button" class="ui-btn-jelly ui-inline" value="Button" />
 *
 * #### Multiline text button
 *
 *      @example
 *      <a href="#" class="ui-btn-jelly ui-multiline ui-inline">A Button<br />Icon</a>
 *
 * ## Options
 *
 * ### Icons
 ...
 *
 *      @example
 *      <button class="ui-btn-jelly ui-more ui-icon-overflow">More Options</button>
 *
 * Samsung Gear Web UI Framework supports 3 icon css styles:
 *
 *  - ui-icon-detail
 ...
 *
 *      @example
 *      <button class="ui-btn-jelly" disabled="disabled">Button disabled</button>
 *
 * ### Inline
 *
 * If you want to make inline button, add class *ui-inline* to button element:
 *
 *      @example
 *      <button class="ui-btn-jelly ui-inline">Inline button</button>
 *
 * ### Multiline
 *
 * If you want to make multiline text button, add *ui-multiline* class
 *
 *      @example
 *      <button class="ui-btn-jelly ui-multiline">Multiline button</button>
 *
 * ### Color theme
 *
 *To optimize color support for the Samsung Gear, the following styles below are supported
 ...
 *      @example
 *      <div class="ui-grid-col-3" style="height:76px">
 *          <button type="button" class="ui-btn-jelly">Button Circle</button>
 *          <a href="#" class="ui-btn-jelly ui-color-red" >A Button Circle</a>
 *          <input type="button" class="ui-btn-jelly ui-color-orange" value="Input Button Circle" />
 *      </div>
 *
 * #### For rows:
 *
 ...
 *      </div>
 *
 * @class ns.widget.wearable.JellyButton
 * @extends ns.widget.wearable.Button
 */
(function (document, ns) {
	"use strict";
				var Button = ns.widget.wearable.Button,
				ButtonPrototype = Button.prototype,
				superInit = ButtonPrototype._init,
				superDestroy = ButtonPrototype._destroy,
				superConfigure = ButtonPrototype._configure,
				engine = ns.engine,
				effectJelly = ns.effects.jellyBackground,
				JellyButton = function () {
					var self = this;

					/**
					 * @property {Object} options Widget configuration
					 * @property {Number} [options.damp=0.3] Jelly effect damping
					 * @property {Number} [options.force=10] Jelly effect force
					 * @property {Boolean} [options.debug=false] Jelly effect debug drawing
					 * @property {Number} [options.smoothing=0.5] Jelly effect line smoothing
					 */
					self.options = self.options || {};
					self.options.damp = 0.3;
					self.options.force = 10;
					self.options.debug = false;
					self.options.smoothing = 0.5;
			 ...
			 * For button, it is an empty object.
			 * @property {Object} events
			 * @member ns.widget.wearable.JellyButton
			 * @static
			 */
			JellyButton.events = Button.events || {};

			/**
			 * Passes options to ns.effects.jellyEffect instance
			 * @method syncOptions
			 * @param {ns.widget.wearable.JellyButton} jellyButtonInstance
			 * @param {Element} element
			 * @member ns.widget.wearable.JellyButton
			 * @private
			 * @static
			 */
			function syncOptions(jellyButtonInstance, element) {
				var options = jellyButtonInstance.options,
					opts = {
						damp: parseFloat(options.damp) || 0,
						force: parseFloat(options.force) || 0,
						smoothing: parseFloat(options.smoothing) || 0.5,
						debug: !!options.debug
					};

				effectJelly.options(element, opts);
			}
			...
			prototype._configure = function (element) {
				effectJelly.on(element);
				if (superConfigure) {
					superConfigure.call(this, element);
				}
				syncOptions(this, element);
			};
			 ...
			 * @protected
			 */
			prototype._setDamp = function (element, value) {
				this.options.damp = value;
				syncOptions(this, element);
			};
			 ...
			 * @protected
			 */
			prototype._setForce = function (element, value) {
			 ...
			 * @protected
			 */
			prototype._setDebug = function (element, value) {
			 ...
			 * @method _setSmoothing
			 * @param {Element} element
			 * @param {Number} value
			 * @member ns.widget.wearable.JellyButton
			 * @protected
			 */
			prototype._setSmoothing = function (element, value) {
				this.options.smoothing = parseFloat(value) || 0.5;
			};
			...
			prototype._destroy = function (element) {
				if (element) {
					effectJelly.off(element);
				}
				if (superDestroy) {
					superDestroy.call(this, element);
				}
				return null;
			};

			JellyButton.prototype = prototype;
			ns.widget.wearable.JellyButton = JellyButton;
...
/*global requestAnimationFrame, ns */
/**
 * #Bubble Menu
 *
 * @author Krzysztof Antoszek <k.antoszek@samsung.com>
 * @author Piotr Karny <p.karny@samsung.com>
 *
 * @class ns.widget.wearable.BubbleMenu
 */
(function (document, ns) {
	"use strict";
				var engine = ns.engine,
			...
			prototype._build = function (element) {
				var options = this.options,
					...
					effectOptions = {
						duration: options.duration,
						distribution: "auto", // for "split"
						animationDelay: options.animationDelay,
						startAngle: options.startAngle,
						direction: options.direction,
						viscosity: options.viscosity,
						centerColor: options.centerColor || null
					};

				switch(options.type) {
					case "custom":
						effectOptions.distribution = "custom";
						break;
					case "path":
						effectOptions.distribution = (typeof optionsDistribution === "number") ? optionsDistribution : "arc";
						break;
					case "frisbee":
						effectOptions.distribution = (typeof optionsDistribution === "number") ? optionsDistribution : "arc";
						effectOptions.animationDelay = 100;
						break;
				}

				this.effect = ns.effects.mergingBubbles.create(element, effectOptions);
			...
			BubbleMenu.prototype = prototype;
