...
class SectionController extends DressElement {
     ...
     */
	onCreated() {
		const $rightAddButton = $(document.createElement('i')),
	...
	_onAddClick(event) {
		...
		const $activeSectionElement = $(sectionChangerComponent.sections[index]);
		if ($target.hasClass('closet-section-add-btn-right')) {
			...
			this._model.insert($activeSectionElement.parent().attr('data-id'),
				packageInfo,
				$activeSectionElement.attr('data-id'));
			index += 1;
		} else {
			...
			this._model.insert($activeSectionElement.parent().attr('data-id'),
				packageInfo,
				$activeSectionElement.prev().attr('data-id'));
		}
