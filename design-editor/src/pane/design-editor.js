...
function moveElement(element, parent, sibling) {
	if (sibling) {
		...
		const $sibling = $(parent).find(`[${  INTERNAL_ID_ATTRIBUTE  }]`);
		if ($sibling.length) {
...
class Model {
	...
	constructor() {
		...
		this._behaviorstate = {
			active: false,
			config: {
				selector: 'script[data-closet-behavior]'
			},
			runner: {
				selector: 'script[data-closet-behavior-runner]'
			}};
	}
	...
	/**
	 * Init state of behaviours
	 * Check if document has tags for behaviors library
	...
	initBehaviors() {
		if (!this._behaviorstate.active) {
			...
			this.behaviorConfig = this._libraryCreator
				.createLibrary(null, this._behaviorstate.config.element, 'js');

			behaviorRunner.setAttribute('data-closet-behavior-runner', true);
			this.behaviorConfig.setAttribute('data-closet-behavior', true);

			behaviorRunner.insertLibContent(() => {console.log('Behaviors copied');});
			behaviorRunner.setAttribute('defer', '');
			this.behaviorConfig.content = this.exportBehaviour();
			this.behaviorConfig.insertLibContent();
			this._DOM.body.appendChild(behaviorRunner.element);
			this._DOM.body.appendChild(this.behaviorConfig.element);
			this._behaviorstate.active = !this._behaviorstate.active;
		}
     ...
     */
	importBehavior() {
		const script = this._behaviorstate.config.element;
		if (script) {
			try {
     ...
     */
	importAnimation() {
		const style = this._DOM.querySelector('style[data-style="animation"]'),
			...
			if (keyFrameMatch) {
				...
				if (keyframeInfo) {
					...
					animationDescription.forEach((animation) => {
						...
						if (animationMatch) {
							const position = animationMatch[1] === '0'
									? keyframeInfo.delay : keyframeInfo.time + keyframeInfo.delay,
					...
					if (!(animationsArray.filter((item) => {
						return item.fromTime === keyframeInfo.delay
						&& item.toTime === keyframeInfo.time + keyframeInfo.delay && item.id === internalId;
					}).length)) {
						animationsArray.push({
							functionName: keyframeInfo.functionName,
							fromTime: keyframeInfo.delay,
							toTime: keyframeInfo.time + keyframeInfo.delay,
							fromKeyframe: keyframesImportObject[keyframeInfo.delay]
								.filter(keyframeItem => keyframeItem.id === internalId)[0],
							toKeyframe: keyframesImportObject[keyframeInfo.time + keyframeInfo.delay]
								.filter(keyframeItem => keyframeItem.id === internalId)[0],
							id: internalId
						});
					}
	...
	importCheckboxStyle() {
		const style = this._DOM.querySelector('style[data-style=checkbox]');
		...
		if (style) {
			...
			const color = [...new Set(style.textContent.match(colorsPattern))],
	...
	replaceElement(id, componentInfo) {
		const parentId = $(findById(this._DOM, id)).parent().attr('data-id');
	...
	_createNewElement(targetDoc, componentInfo) {
		let name = '',
			template = '';
		const fragment = targetDoc.createElement('div');
	...
	wrapChildren(element, wrapString) {
		const $element = $(findById(this._DOM, $(element).attr(INTERNAL_ID_ATTRIBUTE))),
			$wrap = $(wrapString);
		let	$childWrap = null;
		if ($element.length > 0) {
	...
	insert(parent_id, componentPackageInfo, prevSibling_id) {
		const parent = parent_id instanceof HTMLElement ? parent_id : findById(this._DOM, parent_id),
     ...
     * @returns {string}
	 * @todo Change this all script elements to libraries abstraction
     */
	export(iotivityRestImplDisabledScript, deviceHandleModification) {
		...
		if (this._DOM.body.querySelector('r-type')) {
			const rtypeScript = this._DOM.querySelector('script[data-closet-rtype]')
		...
		const style = this._DOM.querySelector('style[data-style="animation"]') || document.createElement('style'),
	...
	undo() {
		...
		if (!this._disableHisotry) {
			...
			if (state) {
				...
				case 'move':
					moveElement(element,
						findById(this._DOM, state.prevParent_id),
						findById(this._DOM, state.prevSibling_id));
					break;
				case 'content':
     ...
     */
	getAllElements() {
		const elementsSelector = componentPackages && componentPackages.getAllComponentsSelector();
     ...
     */
	getTreeElements() {
		const elementsSelector = componentPackages.getAllComponentsSelector(),
			components = {
				root: {
					children: []
				}
		...
		if (elementsSelector) {
			[].map.call(this._DOM.querySelectorAll(elementsSelector), (element) => {
				const parent = closest(element.parentElement, elementsSelector),
					id = element.getAttribute(INTERNAL_ID_ATTRIBUTE),
	...
	getElement(id) {
		let keyframe = null;
		const component = componentPackages.getPackageByElement($(element)),
			element = findById(this._DOM, id),
	...
	getKeyFrameElement(time, id) {
		const keyframe = this.getKeyFrame(time);
		if (id !== null) {
     ...
     */
	addKeyFrame(time, id) {
		let keyframe = this.getKeyFrameElement(time, id);
		if (!keyframe) {
			if (id) {
				keyframe = {
					id: id,
	...
	changeAnimationGroup(oldName, newName) {
		const object = this._animationGroups.get(oldName);
		if (object) {
	...
	setActiveKeyFrame(time) {
		...
		if (this._keyframes[time]) {
			...
			this.checkAnimationsAtTime(time)
				.forEach((animation) => {
					const elementInfo = self.getElement(animation.id),
						styles = Object
							.keys(animation.fromKeyframe.style)
							.concat(Object.keys(animation.toKeyframe.style));
					...
					styles.forEach((styleName) => {
						const fromValue = animation.fromKeyframe.style[styleName] !== undefined
								? parseFloat(animation.fromKeyframe.style[styleName])
								: parseFloat(elementInfo.style[styleName]),
							toValue = animation.toKeyframe.style[styleName] !== undefined
								? parseFloat(animation.toKeyframe.style[styleName])
								: parseFloat(elementInfo.style[styleName]);
	...
	removeBehavior(id, number) {
		...
		this.behaviorConfig.content = this._behaviors;
		this.behaviorConfig.insertLibContent();
		this.dirty();
	}
	...
	insertStyles (style, content) {
		const styleEl = this._DOM.querySelector('style[data-style=checkbox]');
