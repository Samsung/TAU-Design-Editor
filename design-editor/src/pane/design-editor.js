...
class Model {
     ...
     */
	importBehavior() {
		const script = this._DOM.querySelector('script[data-closet-behavior]');
		if (script) {
			try {
	...
	importCheckboxStyle() {
		...
		if (style) {
			...
			const img = [...new Set(style.textContent.match(imgPattern))];
			const selector = [...new Set(style.textContent.match(classpattern))];
			if (color.length !== 0) {
	...
	export(iotivityRestImplDisabledScript, deviceHandleModification) {
		...
		if (this._DOM.body.querySelector('r-type')) {
			const rtypeScript = this._DOM.querySelector('script[data-closet-rtype]') || document.createElement('script');
		...
		projectManager.createLibFromTemplate('behaviour.js', () => {
			...
			this.checkAnimationsAtTime(time)
				.forEach((animation) => {
					...
					styles.forEach((styleName) => {
						const fromValue = animation.fromKeyframe.style[styleName] !== undefined ? parseFloat(animation.fromKeyframe.style[styleName]) :
