...
class Interaction {
	...
	setInteraction($selectedLayerElement, contentElement) {
		const toolbarElement = $('closet-toolbar-container-element')[0];
	...
	setDraggable() {
		let icon = null;
		const elementDetected = ElementDetector.getInstance().detect(this._$contentElement),
		...
		if (elementDetected) {
			...
			if (options.draggable === true) {
				...
				this._$selectedLayerElement.draggable({
					...
					scroll: false,
					helper: function () {
						const $helperEl = $('<div class="closet-component-drag-helper closet-drag-helper-small"></div>');
	...
	_onDragStart(event, ui) {
		...
		const offsetFromParent = $contentElement.position();
     ...
     */
	_onDrag(event, ui) {
		const $contentElement = this._$contentElement,
     ...
     */
	_onResizeStart() {
		const elem = this._$contentElement[0];
	...
	onComponentDragged(event, componentPackageInfo, activeElementInfo) {
		...
		const designEditor = AppManager.getActiveDesignEditor();
