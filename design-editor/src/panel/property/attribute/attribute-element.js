...
class Attribute extends DressElement {
		...
		if (designEditor && _selectedElementId) {
			...
			if (attributes) {
				attributes.forEach((attributeObject) => {
					const  $iframeElement = iframeElement && iframeElement.jquery ?
							iframeElement :
							$(iframeElement),
	...
	_applyBackgroundImageInfo(fileName) {
		...
		if (!fileName || fileName === 'none') {
			...
			if (urls.length) {
				const url = urls[0].replace(/url\(([^)]+)\)/g, '$1');
				if (url.length) {
					const relative = url
						// eslint-disable-next-line max-len
						.replace(new RegExp(`.+${  utils.checkGlobalContext('globalData').basePath.replace(/\//g, '/')  }(.+)`, 'g'), '$1');
					if (relative.length) {
						const clean = relative.replace(/\/+/g, '/').replace(/('|")/g, '').trim();
					...
					utils.checkGlobalContext('writeFile')(
						...
						() => {
							AppManager.getActiveDesignEditor()
								.getModel()
								.updateStyle(this._selectedElementId, 'backgroundImage', `url("${  readpath  }")`);
	...
	_onChange3dWall() {
		const innerScript = ['<div class="inner1">', '<div class="inner2">', '<div class="inner3">'];
		let wallScript = '<div class="wrapper"><div class="outer">',
			cnt = 0;
		const row = 2;
	...
	_onYoutubeChange3dWall() {
		...
		for (let i = 0; i < wallArray.length; i = i + 2) {
	...
	_onParserUrl(callback) {
		...
		fs.readFile(filePath, 'utf8', (err, data) => {
			...
			if (data.length > 0) {
				...
				for (let uIndex = 0; uIndex < urlArray.length; uIndex++) {
					for (let tIndex = 0; tIndex < thumbArray.length; tIndex++) {
