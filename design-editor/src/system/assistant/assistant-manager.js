...
class AssistantManager {
	...
	_onWizardAccepted(codeInfo) {
		const codeToBeInserted = (() => {
			switch (codeInfo.type) {
			case 'instance':
				return this._codeGenerator.getInstance(codeInfo.element, codeInfo.info, this._model);
			case 'tau-widget':
				return this._codeGenerator.getTAUWidget(codeInfo.element, codeInfo.info, this._model);
			case 'listener':
				return this._codeGenerator.getEventListener(codeInfo.element, codeInfo.info, this._model);
			case 'transition':
				return this._codeGenerator.getPageTransition(codeInfo.element, codeInfo.info, this._model);
			case 'popup':
				return this._codeGenerator.getPopupOpen(codeInfo.element, codeInfo.info, this._model);
			}
		})();

		this._assistantView.insertCode(codeToBeInserted);
	}
	...
	_onOpenAssistantCodeWizard(options = {}) {
		options.element = options.element || this._designEditor && this._designEditor.getSelectedElement();

		if (options.element) {
			this._assistantWizard.open(
				options.element,
				this._codeGenerator.getInstanceListFromMap(options.element)
			);
		}
	}
}
